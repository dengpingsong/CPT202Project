...
async function fetchProfile() {
  ...
  try {
    ...
    if (p) {
      ...
      email.value = p.email || ''
      programme.value = p.programme || ''
      enrollmentDate.value = formatDateForInput(p.enrollmentDate)
      phone.value = p.phone || ''
...
async function handleSave() {
  ...
  if (enrollmentDate.value) {
    ...
    if (new Date(`${enrollmentDate.value}T00:00:00`) > today) {
      profileStatus.value = 'Enrollment date cannot be in the future.'
      profileStatusType.value = 'error'
      return
    }
  }
...
async function handleChangePassword() {
  ...
  try {
    await studentApi.changePassword(oldPassword.value, newPassword.value)
