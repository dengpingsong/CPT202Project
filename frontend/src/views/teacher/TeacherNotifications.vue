...
import { teacherApi, getCurrentUser } from '../../utils/api'
...
const readStorageKey = computed(() => {
  ...
  return `teacherNotificationRead:${user.userId || user.username || 'current'}`
})
...
async function loadNotifications() {
  loading.value = true
  try {
    const res = await teacherApi.listNotifications()
    const requests = Array.isArray(res.data) ? res.data : []
    notifications.value = buildNotifications(requests)
  } catch (e: any) {
